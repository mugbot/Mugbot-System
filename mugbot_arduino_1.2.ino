...
//頭部レンジマグの代替品クルールボウル対応のために目の角度が水平より
//上がらないように調整
...
void loop() {
  if (Serial.available() > 0) {
    ...
    switch (ch) {
      ...
      case 'x': //Rasberry Piからｘを受け取ると首を左右に運動。
        head_servo.write( g );
      ...
      case 't': //Rasberry Piからtを受け取ると口が点滅し、発話時の動作を行う。
        ...
        for ( pos = eye_default; pos > 80; pos -= 1 ) {
          eye_servo.write( pos );
          delay( 15 );
        } ;
      ...
      case 's': //Rasberry Piからsを受け取ると「悲しい」のアクションを行う。
        for ( pos = eye_default; pos > 75; pos -= 1 ) {
          eye_servo.write( pos );
          delay( 15 );
        } ;

        for ( pos = eye_light_default; pos > 10; pos -= 1 ) {
      ...
      case 'b': //Rasberry Piからbを受け取ると「びっくり」のアクションを行う。
        for ( pos = eye_default; pos < 95; pos += 1 ) {
          eye_servo.write( pos );
          delay( 15 );
        } ;
      ...
      case 'e': //Rasberry Piからeを受け取ると「エキサイト」のアクションを行う。
        for ( ch = 0; ch < 5; ch += 1 ) {
          digitalWrite(mouse, HIGH);
          ...
          analogWrite(r_eye, eye_light_default);
          delay( 100 );
          eye_servo.write(random(70, eye_default) );
          delay( 100 );
          head_servo.write(random(10, 170) );
          delay( 100 );
          analogWrite(l_eye, eye_light_default);
          //analogWrite(r_eye, eye_light_default);
          delay( 100 );
